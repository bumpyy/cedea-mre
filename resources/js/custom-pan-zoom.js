...
(function () {
    ...
    window.customInteractiveImage = function (
        imageUrl,
        imageId,
        doubleClickZoomLevel = 3
    ) {
        return {
            imageUrl: imageUrl,
            imageId: imageId,
            ...
            fitToContainer() {
                ...
                const centeredPanX =
                    (containerWidth - imageWidth * newScale) / 2;
                const centeredPanY =
                    (containerHeight - imageHeight * newScale) / 2;
            ...
            doubleClickZoom(e) {
                ...
                const isNearFitted =
                    near(this.scale, this.fittedScale) &&
                    near(this.panX, this.fittedPanX, 1) &&
                    near(this.panY, this.fittedPanY, 1);

                if (isNearFitted) {
                    // Zoom to configurable level at the exact click position using top-left origin math
                    const newScale = Math.min(
                        this.maxScale,
                        this.doubleClickZoomLevel
                    );
