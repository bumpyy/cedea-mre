...
<input data-slot="otp-input"
    ...
    x-ref="$name" type="text" inputmode="numeric" required maxlength="1" x-on:input="handleInput($el)"
    x-on:keydown.enter="handleInput($el)" x-on:paste="handlePaste($event)" {{-- traits delete key as backspace --}}
    x-on:keydown.delete.prevent="await handleBackspace($event)" {{-- handle backspace key --}}
    x-on:keydown.backspace.prevent="await handleBackspace($event)" {{-- accessibilty addons --}} autocomplete="one-time-code"
