...
}" x-id="['modal']" x-bind:data-modal-open="isOpen" x-bind:data-modal-id="modalId" @if ($ariaLabelledby) aria-labelledby="{{ $ariaLabelledby }}"
@elseif($hasHeading) x-bind:aria-labelledby="$id('modal') + '-heading'" @endif
    {{ $attributes->class(['modal-component', $displayClasses, 'hidden' => !$visible]) }}
    x-on:keydown.window="handleEscapeKey($event)">
    {{-- Trigger Element --}}
    @if ($trigger)
        <div x-on:click="open()" {{ $trigger->attributes->class('modal-trigger cursor-pointer') }}>
            {{ $trigger }}
        </div>
    @endif
    ...
    <template x-teleport="body">
        <div class="fixed inset-0 overflow-y-auto" x-show="isOpen" aria-modal="true" role="dialog"
            style="display: none;z-index:9999">
            {{-- Backdrop --}}
            <x-ui.modal.backdrop />

            {{-- Modal Container --}}
            <div data-slot="modal-container" @if ($slideover) data-slideover="true" @endif
                x-on:click="handleBackdropClick($event)" @class([
                    'relative flex min-h-full z-50 items-center justify-center',
                    'p-4' => !$slideover && $width !== 'screen',
                    'items-start pt-16' =>
                        $width !== 'screen' && ($position === 'top' && !$slideover),
                    'items-end pb-16' => $position === 'bottom' && !$slideover,
                    ' overflow-x-hidden' => $slideover,
                ])>
                {{-- Modal Content --}}
                <div data-slot="modal-contents" x-ref="modalContent" x-show="isOpen"
                    @if ($animation === 'scale') x-transition:enter="transition ease-out duration-200"
                        ...
                        <div @class([
                            ...
                            @if ($hasIcon)
                                <div class="mr-4 flex-shrink-0">
                                    <div @class(['rounded-full p-2', $iconColorClass])>
                                        <x-ui.icon :name="$icon" />
                                    </div>
                                </div>
                            @endif
                            ...
                            {{-- Close Button --}}
                            @if ($closeButton)
                                <x-ui.button class="rounded-field bg-black/5 dark:bg-white/5"
                                    x-on:click="$data.close();" variant="none" {{-- prevent the icon to be aware of our current icon --}} :icon="null"
                                    size="sm" icon-after="x-mark" />
                            @endif
                        </div>
                    @endif
                    ...
                    @if ($hasSlot)
                        <div @class([
                            ...
                            'overflow-y-auto' =>
                                $slideover || $width === 'screen' || $stickyFooter || $stickyHeader,
                            'max-h-[calc(100vh-13.8rem)]' =>
                                ($stickyHeader || $stickyFooter) && !$slideover && $width !== 'screen',
                        ])>
                            {{ $slot }}
                        </div>
                    @endif
