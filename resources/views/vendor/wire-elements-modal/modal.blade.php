<div>
    ...
    <div class="fixed inset-0 z-10 overflow-y-auto" x-data="LivewireUIModal()" x-on:close.stop="setShowPropertyTo(false)"
        ...
        <div class="flex min-h-dvh items-end justify-center px-4 pb-10 pt-4 text-center sm:block sm:p-0">
            ...
            <div class="relative inline-block w-full transform overflow-hidden text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:align-middle"
                id="modal-container" x-show="show && showActiveComponent" x-transition:enter="ease-out duration-300"
                x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave="ease-in duration-200"
                x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100"
                x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"
                x-bind:class="modalWidth" x-trap.noscroll.inert="show && showActiveComponent" aria-modal="true">
