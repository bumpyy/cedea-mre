...
namespace App\Filament\Resources\SubmissionAreas\Tables;
            ...
            ->toolbarActions([
                Action::make('assign_5')
                    ...
                    ->action(function () {
                        ...
                        $pendingSubmissions = Submission::where('admin_id_area', $admin->id)
                        ...
                        if ($pendingSubmissions < 5) {
                            Submission::where('status', SubmissionStatusEnum::ACCEPTED)
                                ->whereNull('admin_id_area')
                                ->where(function (Builder $query) {
                                    $query->whereNull('store_area')
                                        ->orWhere('store_area', '');
                                })
                                ...
                                ->update([
                                    'admin_area_assigned_at' => now(),
                                    'admin_id_area' => $admin->id,
                                ]);
                        }
                    })
